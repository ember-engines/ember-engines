  ...
  extend(options) {
    ...
    */
    options.contentFor = function(type, config) {
      if (type === 'head' && this.app.options.storeConfigInMeta) {
    ...
    */
    options.getEngineConfigContents =
      options.getEngineConfigContents ||
      function() {
        var storeConfigInMeta = this.app.options.storeConfigInMeta;
        var configTemplatePath =  path.join(__dirname, storeConfigInMeta ? '/engine-config-from-meta.js' : '/engine-config.js');
        var configTemplate = fs.readFileSync(configTemplatePath, { encoding: 'utf8' });

        configTemplate = configTemplate.replace('{{MODULE_PREFIX}}', options.name);

        if (storeConfigInMeta) {
          return configTemplate;
        } else  {
          return configTemplate.replace('{{ENGINE_CONFIG}}', JSON.stringify(this.engineConfig(this.app.env, {})));
        }
      };
