...
describe('appendCompactReexportsIfNeeded', function () {
  ...
  it('works', function () {
    expect(appendCompactReexportsIfNeeded([])).to.eql([
      BABEL_COMPACT_REEXPORTS_PLUGIN,
    ]);

    expect(
      appendCompactReexportsIfNeeded([BABEL_COMPACT_REEXPORTS_PLUGIN]),
    ).to.eql([BABEL_COMPACT_REEXPORTS_PLUGIN]);

    expect(
      appendCompactReexportsIfNeeded([
        BABEL_COMPACT_REEXPORTS_PLUGIN,
        BABEL_COMPACT_REEXPORTS_PLUGIN,
      ]),
    ).to.eql([BABEL_COMPACT_REEXPORTS_PLUGIN, BABEL_COMPACT_REEXPORTS_PLUGIN]);
    ...
    expect(appendCompactReexportsIfNeeded(['a', 'b', 'c', foo])).to.eql([
      'a',
      'b',
      'c',
      foo,
